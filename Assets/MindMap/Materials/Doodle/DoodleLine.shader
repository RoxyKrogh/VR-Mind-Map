...
#define OUT_TRI(vert1, vert2, vert3, vert4) {																				\
					...
					float3 normal = normalize(cross((vert2 - vert1).xyz, (vert3 - vert1).xyz));								\
					out1.normal = normal;																					\
					out2.normal = normal;																					\
					out3.normal = normal;																					\
					out4.normal = normal;																					\
				...
				float4 right0 = float4(cross(input[0].tangent.xyz, norm0.xyz), 0);
				float4 right1 = float4(cross(input[1].tangent.xyz, norm1.xyz), 0);
			...
			{
				fixed4 col = _LineColor;  // apply the line color
				float3 ls = _WorldSpaceLightPos0.xyz; // get the light direction
				float minLight = 0.2;  // minimum light value
				col.rgb *= max(minLight, dot(ls, i.normal)); // apply the light value to the color
				return col;
			}
			ENDCG
		}
